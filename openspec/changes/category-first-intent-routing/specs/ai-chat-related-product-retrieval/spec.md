...
He thong chatbot SHALL tim va de xuat mon theo thu tu: category match (neu co category intent) -> exact product match -> related match -> safe random response.
...
#### Scenario: Co category intent thi khong duoc leak khac category
- **WHEN** user yeu cau theo category xac dinh (vi du "lien quan den hat")
- **THEN** danh sach `matchedProductIds` va action `xem/mua` phai chi chua san pham trong category do
